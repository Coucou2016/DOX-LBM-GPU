...
快速相图得到 1 VA / 3 Non-VA。本报告汇总证据链、图件与局限；不是三维 DOX 孪生复现。
...
- 默认环相图稳定为 1 VA / 3 Non-VA；pytest 42 passed。
...
右栏（快速相图计数）：钉扎环 2×2 快扫，期望 1 VA / 3 Non-VA。
若变成 0/4，优先怀疑几何放不下波长；若变成 4/4，优先怀疑终点过松。
...
### 图3. 钉扎环 λ_fib × D_fib 诱发性相图（1 VA / 3 Non-VA）

![钉扎环 λ_fib × D_fib 诱发性相图（1 VA / 3 Non-VA）](../papers/figures/fig_phase_diagram.png)

**图注：** 暖色=VA，冷色=Non-VA；唯一 VA 格点为 λ=0.01 且 D↓90%。
...
唯一 VA：λ=0.01 × D↓90%。此时 CV≈√0.1×0.70≈0.22 mm/ms，波长≈55 mm < 107 mm，几何允许折返，
且周期必需准则看到再兴奋（extra / probes）。
λ=0.3 两格：接近阻滞 → Non-VA；D↓30%×λ=0.01：波长仍偏长 → Non-VA。
...
- 完整 4×3 相图、Niederer/openCARP/MonoAlg3D 交叉验证仍为 P2（待补充）。
